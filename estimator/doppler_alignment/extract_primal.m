function [R, status, SVR] = extract_primal(dual_sol)
...
% zero) and that it is well separated from the second smallest (i.e. rank is tight)
zero_check = s(end)<1e-3 && (s(end)/s(end-1))<1e-6;  % --sbs
